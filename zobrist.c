...
uint64_t castlingRightsHash[NUM_COLORS][NUM_SIDES];
uint64_t enPassantFileHash[NUM_FILES];
uint64_t pieceHashForSquares[NUM_PIECES][SIZE];  // 120 pieces squares, 12 piece types (P, p, B, p, ...) => 1440 random hash
...
void initializeZobristHashes() {
    ...
    for (int color = 0; color < NUM_COLORS; color++) {
        for (int castlingSide = 0; castlingSide < NUM_SIDES; castlingSide++) {
